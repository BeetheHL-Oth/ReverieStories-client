...
export default function Home() {
  ...
  const { stories, storyCount, loading, error } = useSelector(
    (state) => state.stories,
  );
  ...
  useEffect(() => {
    dispatch(fetchStories({ page, tag, search }));
  }, [dispatch, page, tag, search]);

  // async function fetchStories() { //! change this and remove into slicer
  //   try {
  ...
  return (
    <>
      <div className="w-full bg-amber-50 text-slate-900 flex flex-col mt-15 p-5">
        <div className="flex w-9/10 justify-between items-center">
          <form className="pb-3 px-20 w-full">
            <input
              type="text"
              value={search}
        ...
        <div className="flex gap-8 px-10 w-full h-fit bg-amber-50 text-slate-900">
          <div className="flex flex-col w-3/4 h-200 outline-1 outline-neutral-200 shadow-xl bg-stone-50">
          ...
          : <div className="flex flex-col w-1/4 h-200 bg-amber-50 gap-8 p-5 overflow-y-auto">
